...
<div id="section">
                ...
                <c:when test="${map.list != null && !map.list.isEmpty()}">
                    <c:forEach var="noti" items="${map.list}">
                        <tr>
                            ...
                            <td class="retitle">
                                <a href="notice3?ann_seq=${noti.ann_Seq}">${noti.ann_Title}</a>
                            </td>
                            <td>${noti.ann_Regi}</td>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${noti.ann_attach != null}">
                                        <a href="${pageContext.request.contextPath}/download?filePath=${noti.ann_attach}">파일 다운로드</a>
                                    </c:when>
                                    <c:otherwise>
                                        없음
                                    </c:otherwise>
    ...
    int countPerPage = Integer.parseInt(request.getAttribute("countPerPage").toString());
