            ...
            <div id="movepage">
                ...
                <c:if test="<%= sec_first != 1 %>">
                	<span class="material-symbols-outlined">
						<a href="loan?page=<%= sec_first - 1 %>&user_seq=${login.user_seq}">chevron_left</a>
					</span>
				</c:if>
				...
				<c:forEach var="i" begin="<%= sec_first %>" end="<%= sec_last %>">
					...
					<c:if test="${i eq param.page}"> 
						<a href="loan?page=${i}&user_seq=${login.user_seq}" id="page" class="chap"><strong>${i}</strong></a>
					</c:if>
					<c:if test="${i != param.page}"> 
						<a href="loan?page=${i}&user_seq=${login.user_seq}" id="page" class="chap">${i}</a>
					</c:if>
				...
				<c:if test="<%= sec_last != lastPage %>">
                	<span class="material-symbols-outlined">
                		<a href="loan?page=<%= sec_last + 1 %>&user_seq=${login.user_seq}">chevron_right</a>chevron_right
                	</span>
                </c:if>
            </div>
        </section>
